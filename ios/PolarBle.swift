    ...
    PolarBleApiDeviceInfoObserver, PolarBleApiDeviceFeaturesObserver, PolarBleApiDeviceHrObserver
{
        ...
        do {
            ...
            searchDisposable = api.searchForDevice().subscribe { event in
                ...
                case .error(let error):
                    reject(nil, error.localizedDescription, error)
                case .next((let deviceId, let address, let rssi, let name, let connectable)):
                    self.sendEvent(
    ...
    func deviceConnecting(_ identifier: PolarDeviceInfo) {
        sendEvent(withName: PolarEvent.deviceConnecting.rawValue, body: identifier)
    }

    func deviceConnected(_ identifier: PolarDeviceInfo) {
        sendEvent(withName: PolarEvent.deviceConnected.rawValue, body: identifier)
    }

    func deviceDisconnected(_ identifier: PolarDeviceInfo) {
        sendEvent(withName: PolarEvent.deviceDisconnected.rawValue, body: identifier)
    }

    func batteryLevelReceived(_ identifier: String, batteryLevel: UInt) {
        sendEvent(withName: PolarEvent.batteryLevelReceived.rawValue, body: batteryLevel)
    }
    ...
    func hrValueReceived(_ identifier: String, data: PolarHrData) {
        sendEvent(
            ...
            body: ["identifier": identifier, "data": data])
    }
